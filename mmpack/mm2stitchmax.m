function mm2stitchmax(ch,refch,overlap,singleslice,useapproxpos)
...
for n=1:ch
    ...
    for i=1:length(files)
        ...
        im=imrotate(imread(files{i},n),-90)'; %read image.
        imwrite(im,['stitchchannel/ch',num2str(n),'/','MAX_Pos',pos{i},'_',num2str(xy(i,1),'%.3u'),'_',num2str(xy(i,2),'%.3u'),'.tif']);
    end
end
...
if useapproxpos==0
    for i=1:length(uniqpos)
        ...
        MIJ.run('Grid/Collection stitching', ['type=[Filename defined position] ',...
            ...
            'file_names=',['MAX_Pos',uniqpos{i},'_{xxx}_{yyy}','.tif '],...
    ...
    for n=1:ch
        ...
        for i=1:length(uniqpos)
            ...
            MIJ.run('Grid/Collection stitching', ['type=[Filename defined position] ',...
                ...
                'computation_parameters=[Save computation time (but use more RAM)] ',...
                'image_output=[Fuse and display]']);
            MIJ.run('Save',['Tiff..., path=[',pwd,'\ch',num2str(n),'stitched\MAX_Pos',uniqpos{i},'.tif]']);
            MIJ.run("Close All")
        end
    end
...
for i=1:length(uniqpos)
    im=imread(['ch1stitched/MAX_Pos',uniqpos{i},'.tif']);
    if singleslice==0
        imwrite(im,['stitchedMAX_Pos',uniqpos{i},'.tif']);
        for n=2:ch
            im=imread(['ch',num2str(n),'stitched/MAX_Pos',uniqpos{i},'.tif']);
            imwrite(im,['stitchedMAX_Pos',uniqpos{i},'.tif'],'WriteMode','Append');
        end
    else
        for n=1:ch
            im=imread(['ch',num2str(n),'stitched/MAX_Pos',uniqpos{i},'.tif']);
            imwrite(im,['stitchedMAX_Pos',uniqpos{i},'_ch',num2str(n),'.tif']);
        end
    end
end
