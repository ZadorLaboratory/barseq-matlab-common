function mmstitchgenessingleplane(ch,refch,overlap,filenum)
...
mkdir(['stitchchannel',num2str(filenum)]);
...
for n=1:ch
    mkdir(['stitchchannel',num2str(filenum),'/ch',num2str(n)]);
    for i=1:length(files)
        cd(files{i});
        cd original;
        f=dir('*geneseq*.tif');
        ...
        imwrite(im,['../../stitchchannel',num2str(filenum),'/ch',num2str(n),'/',files{i},'.tif']);
        cd ../../
    end
end
...
Miji(false);
cd(['stitchchannel',num2str(filenum)]);
