function niesinglecyclestitch(ch,refch,overlap,cropfrac)
...
for i=1:length(files)
    sipxy(i,:)=textscan(files{i},'%*s %s %u %u %*s',1,'Delimiter',{'_','.'});
...
for n=1:ch
    ...
    for i=1:length(files)
        ...
        imwrite(im,[foldername,'/ch',num2str(n),'/',files{i}]);
