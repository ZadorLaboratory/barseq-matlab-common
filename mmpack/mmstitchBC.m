function mmstitchBC(ch,refch,overlap)
...
mkdir stitchchannelBC
for n=1:ch
    mkdir(['stitchchannelBC/ch',num2str(n)]);
    for i=1:length(files)
        cd(files{i});
        cd original;
        f=dir('*bc*.tif');
        f=sort_nat({f.name});
        im=imrotate(imread(f{1},n),180); %read and invert image.
        imwrite(im,['../../stitchchannelBC/ch',num2str(n),'/',files{i},'.tif']);
        cd ../../
    end
end
