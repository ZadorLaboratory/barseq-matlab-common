function mmbasecallgenesoptimizelowres(rolthreshall,psf,bgn)
%optimize rolthresh for mmbasecallgenes. This version works better for 20x
%because it doesn't try to remove adjacent rolonies.
...
iplot=ceil(size(rolthreshall,1)/4*[1 2 3 4]);
for n=1:4
    a=im(:,:,n);
    maxthresh=prctile(a(:),99.9);
    figure;
    s=zeros(size(rolthreshall,1),1);
    for i=1:size(rolthreshall,1)
        if ismember(i,iplot)
