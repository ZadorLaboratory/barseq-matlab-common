function projplot1(data,vdir,paramstruct) 
%
% LXS Tools
% Edited from the PROJPLOT1SM
% - add linesize option
%
    ...
    hold on ;
      if ndo > 0 ;    %  then add data to plot
        ...
        if ibigdot == 1 ;   %  plot deliberately large dots
          plot(vproj(vindol),hts,[dotcolor 'o'],'MarkerSize',1,'LineWidth',linesize) ;
        else ;    %  use input marker
          plot(vproj(vindol),hts,[dotcolor markerstr],'LineWidth',linesize) ;
        end ;
...
elseif indivplotflag == 1 ;    %  then need to do individual plot calls
  ...
  if ~(isubpopkde == 2) ;    % then plot full data KDE
    plot(kdexgrid,kde,[kdecolor '-'],'LineWidth',linesize) ;
  end ;
  if  (isubpopkde == 1)  |  (isubpopkde == 2) ;    %  then plot subpop KDEs
    hold on ;
      for isp = 1:nsubpop ;
        plot(kdexgrid,mspkde(:,isp),'-','Color',subpopcolor(isp,:),'LineWidth',linesize) ;
      end ;
    hold off ;
  end ;
    axis(vax) ;
    hold on ;
      if ndo > 0 ;    %  then add data to plot
        ...
        for idato = 1:ndo ;
          if ibigdot == 1 ;   %  plot deliberately large dots
            plot(vproj(vindol(idato)),hts(vindol(idato)), ...
                        'o','Color',colmap(vindol(idato),:), ...
                        'MarkerSize',1,'LineWidth',linesize) ;
          else ;    %  use input marker
            plot(vproj(vindol(idato)),hts(idato), ...
                         mmarks(vindol(idato)), ...
                         'Color',colmap(vindol(idato),:),'LineWidth',linesize) ;
          end ;
        end ;
