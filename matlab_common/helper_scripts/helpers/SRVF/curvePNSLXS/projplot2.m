function projplot2(data,mdir,paramstruct) 
%
% LXS Tools
% Edited from PROJPLOT2SM
% - all linesize option
%
...
if nargin > 2 ;   %  then paramstruct is an argument
  ...
  if isfield(paramstruct,'linesize') ;    %  then change to input value
    linesize = getfield(paramstruct,'linesize') ; 
  end ;    
...
if  size(idataconncolor,1) == 1  &  idataconncolor == 2  ;
  ...
  if ndataconn <= size(colmap,1) ;    %  then have enough colors in icolor
    idataconncolor = colmap(1:ndataconn,:) ;
        ...
        disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
        disp('!!!   Warning from projplot2.m:      !!!') ;
        disp('!!!   invalid icolor, corresponding    !!!') ;
...
elseif indivplotflag == 1 ;    %  then need to do individual calls to plot
  ...
  if ibigdot == 1 ;   %  plot deliberately large dots
    plot(mproj(1,1),mproj(1,2),'o','Color',colmap(1,:), ...
                                'MarkerSize',1,'LineWidth',linesize) ;
  else ;    %  use input marker
    plot(mproj(1,1),mproj(1,2),mmarks(1),'Color',colmap(1,:),'LineWidth',linesize) ;
  end ;
    hold on ;
      for idat = 2:n ;
        ...
        if ibigdot == 1 ;   %  plot deliberately large dots
          ...
          plot(mproj(idat,1),mproj(idat,2),mmarks(idat),'Color',colmap(idat,:),'LineWidth',linesize) ;
        end ;
...
if iplotaxes == 1 ;
  hold on ;
    plot([vax(1); vax(2)],[0;0],'k-','LineWidth',linesize) ;
      maxscale = max(abs(vax)) * 2 ;
    plot(maxscale * [-vd2c1(1),vd2c1(1)], ...
         maxscale * [-vd2c1(2),vd2c1(2)],'k-','LineWidth',linesize) ;
  hold off ;
end ;

if iplotdirvec == 1 ;
  hold on ;
    plot([0;1],[0;0],'k-','LineWidth',linesize) ;
    plot([0;vd2c1(1)],[0;vd2c1(2)],'k-','LineWidth',linesize) ;
  hold off ;
end ;
...
if ~isempty(idataconn) ;
  ...
  for idc = 1:ndc ;    %  loop through rows of idataconn
    ph = plot([mproj(idataconn(idc,1),1);mproj(idataconn(idc,2),1)], ...
         ...
         mdataconntype(idc,:),'Color',mdataconncolor(idc,:),'LineWidth',linesize) ;
  end ;
