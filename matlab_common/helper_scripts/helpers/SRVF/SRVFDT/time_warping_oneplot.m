...
if option.showplot == 1
    ...
    for i=1:size(f,2)
    hold on;
    plot(t, f(:,i),'color',colmap(i,:), 'linewidth', 1);
    hold on;
    end 
...
if option.showplot == 1
    subplot(2,3,2)
     for i=1:size(f,2)
    hold on;
    plot((0:M-1)/(M-1), gam(:,i),'color',colmap(i,:), 'linewidth', 1);
    hold on;
    end 
    
    axis square;
    title('Warping functions', 'fontsize', 10);
    
    subplot(2,3,3)
    for i=1:size(f,2)
    hold on;
    plot(t, fn(:,i),'color',colmap(i,:), 'linewidth', 1);
    hold on;
    end 
   
    title(['Warped data, \lambda = ' num2str(lambda)], 'fontsize', 10);
    
    subplot(2,3,4)
    plot(t, mean_f0, 'b-', 'linewidth', 1); hold on;
    plot(t, mean_f0+std_f0, 'r-', 'linewidth', 1);
    plot(t, mean_f0-std_f0, 'g-', 'linewidth', 1);
    title('Original data: Mean \pm STD', 'fontsize', 10);
    
    subplot(2,3,5)
    plot(t, mean_fn, 'b-', 'linewidth', 1); hold on;
    plot(t, mean_fn+std_fn, 'r-', 'linewidth', 1);
    plot(t, mean_fn-std_fn, 'g-', 'linewidth', 1);
    title(['Warped data, \lambda = ' num2str(lambda) ': Mean \pm STD'], 'fontsize', 10);
