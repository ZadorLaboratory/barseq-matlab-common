function [idxall,silh,p]=clustall2(x,dist)
...
%this version does not use multiple comparison 
...
while 1
    ...
    for i=1:size(currgroups,1)
        ...
        if rem(currgroups(i),10)~=0&&sum(idxall==currgroups(i))>size(x,1)/100 % only cluster if the current subcluster was a new cluster and if current cluster size is larger than 1% of whole sample
            ...
            p(n,3)=size(subx,1);
            n=n+1;
            if pval<0.05 % 100 being the max number of splits required to produce clusters each with >=1% of all cells %/(size(currgroups,1)+sum(rem(currgroups,10)>0))
