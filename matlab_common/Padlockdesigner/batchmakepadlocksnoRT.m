function [allprimers,allpadlocks,allTm,allloc,allgenes]=batchmakepadlocksnoRT(accnum, barcodes,giiidx,idx)
...
for n=1:length(accnum)
    if idx(n)
        ...
        if err~=1
            [primer,padlock,Tm,loc]=designdnapadlocksnoRT(sequence,barcodes(giiidx(n),:));
