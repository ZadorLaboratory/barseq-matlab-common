function validtargetsnm32=validateshortpadlockdesigns(padlocks,names)
%blast the primers/padlocks on NCBI against mouse genome + transcriptome,
%download the blast results and read them.

%write arms into a fasta file
names=cellfun(@(x)x(x~=' '),names,'UniformOutput',false);

armidx=cellfun(@(x)regexpi(x,'GGACGGACTCGACCGTCG......'),padlocks);
arms=cell(length(padlocks),1);
for i=1:length(padlocks)
    arms{i}=[padlocks{i}(armidx(i)+24:end),padlocks{i}(1:armidx(i)-1)];
end
...
!blastn -query arms1.fasta -out results.txt -import_search_strategy NMonly.asn -remote -outfmt "6 qseqid sseqid qstart btop evalue"
...
% 3. The matching bases of each arm has a Tm of at least 32
...
for n=1:size(validtargetsnm32,1)
    data=getgenbank(validtargetsnm32{n,2});
    validtargetsnm32{n,3}=data.Definition;
end
